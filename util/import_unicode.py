...
with open (sys.argv[3], "r") as infile:
    ...
    for line in infile:
        ...
        if sp[2] == "U":
            ...
            print(f"Unknown joining type '{sp[2]}'")
            exit (1)
with open (sys.argv[1], "r") as infile:
    ...
    for line in infile:
        ...
        if sp[2] not in [
            ...
            print(f"WARNING: Unknown type {sp[2]}")
        ...
        if sp[1].startswith ("ARABIC LETTER "):
            ...
            if arabname.endswith (" ISOLATED FORM"):
                arabname = arabname[:len (arabname) - len (" ISOLATED FORM")]
                form = 1
            if arabname.endswith (" FINAL FORM"):
                arabname = arabname[:len (arabname) - len (" FINAL FORM")]
                form = 2
            if arabname.endswith (" MEDIAL FORM"):
                arabname = arabname[:len (arabname) - len (" MEDIAL FORM")]
                form = 3
            if arabname.endswith (" INITIAL FORM"):
                arabname = arabname[:len (arabname) - len (" INITIAL FORM")]
                form = 4
...
for x, value in arabicsubst.items():
    try:
        if value['join'] == "DUAL":
